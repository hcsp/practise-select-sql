...
// +----+----------+------+----------+
    public static List<User> getUsersByPageOrderedByIdDesc(Connection databaseConnection, int pageNum, int pageSize) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select id,name,tel,address from user u\n" +
                "order by u.id desc\n" +
                "limit ? ,?")) {
...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select g.id as id,g.name,sum(goods_num)*goods_price as gmv from `order` o \n" +
                "inner join goods g\n" +
                "on o.goods_id = g.id\n" +
                "group by id \n" +
                "order by gmv desc")) {
