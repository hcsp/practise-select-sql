...
// +------------+-----------------+------------------+---------------------+-------------------------------+
     ...
     * @param databaseConnection SQL connection
     * @return 有多少用户买过这个商品
     */
...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select GOODS_ID, GOODS.NAME, SUM(GOODS_NUM*GOODS_PRICE) as total from \"ORDER\" \n" +
                "join GOODS\n" +
                "on \"ORDER\".GOODS_ID = GOODS.ID\n" +
                "group by GOODS_ID order by total desc")) {
            ResultSet resultSet = statement.executeQuery();
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select \"ORDER\".ID, USER.NAME, GOODS.NAME, GOODS_NUM * GOODS_PRICE from \"ORDER\"  \n" +
                "    join GOODS\n" +
                "    on \"ORDER\".GOODS_ID = GOODS.ID\n" +
                "    join USER\n" +
