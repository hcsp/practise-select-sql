...
// +----+----------+------+----------+
    public static List<User> getUsersByPageOrderedByIdDesc(Connection databaseConnection, int pageNum, int pageSize) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select ID,NAME,TEL,ADDRESS from USER order by ID desc limit ? offset ?")) {
            ...
            statement.setObject(2, (pageNum - 1) * pageSize);
            ResultSet resultSet = statement.executeQuery();
            ...
            while (resultSet.next()) {
                ...
                user.tel = resultSet.getNString(3);
...
//  +----+--------+------+
    public static List<GoodsAndGmv> getGoodsAndGmv(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select GOODS.ID as ID,NAME,sum(GOODS_NUM*GOODS_PRICE) as GMV from GOODS join \"ORDER\" on GOODS.ID = \"ORDER\".GOODS_ID group by GOODS.ID order by GMV desc")) {
            ResultSet resultSet = statement.executeQuery();
...
// +----------+-----------+------------+-------------+
    public static List<Order> getInnerJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select \"ORDER\".ID as ORDER_ID,USER.NAME as USER_NAME,GOODS.NAME as GOODS_NAME,GOODS_PRICE*GOODS_NUM as TOTAL_PRICE from GOODS INNER JOIN  \"ORDER\" on GOODS.ID = \"ORDER\".GOODS_ID INNER JOIN USER on \"ORDER\".USER_ID = USER.ID")) {
            ResultSet resultSet = statement.executeQuery();
            List<Order> orderList = new ArrayList<>();
            while (resultSet.next()) {
                Order order = new Order();
                order.id = resultSet.getInt(1);
                order.userName = resultSet.getNString(2);
                order.goodsName = resultSet.getNString(3);
...
// +----------+-----------+------------+-------------+
    public static List<Order> getLeftJoinOrders(Connection databaseConnection) throws SQLException {
        try (PreparedStatement statement = databaseConnection.prepareStatement("select \"ORDER\".ID as ORDER_ID,USER.NAME as USER_NAME,GOODS.NAME,GOODS_PRICE*GOODS_NUM as TOTAL_PRICE from \"ORDER\" left join GOODS  on \"ORDER\".GOODS_ID = GOODS.ID left join USER on \"ORDER\".USER_ID = USER.ID")) {
            ResultSet resultSet = statement.executeQuery();
            List<Order> orderList = new ArrayList<>();
            while (resultSet.next()) {
                Order order = new Order();
